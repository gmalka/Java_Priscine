...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public Optional<Message> findById(Long id) {
        ...
        try {
            pst = connection.prepareStatement("SELECT * FROM chat.messages WHERE id = " + id);
            rs = pst.executeQuery();
            rs.next();
            LocalDateTime time;
            try {
    ...
    {
        try {
            PreparedStatement pst = connection.prepareStatement("SELECT * FROM chat.users WHERE id = " + id);
    ...
    {
        try {
            PreparedStatement pst = connection.prepareStatement("SELECT * FROM chat.chatroom WHERE id = " + id);
            ResultSet rs = pst.executeQuery();
            rs.next();
